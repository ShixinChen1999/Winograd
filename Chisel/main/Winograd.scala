import chisel3._
import chisel3.core.Module.clock
...
class MUL_module extends Module with Config{
  ...
  /*
    ...
    when(cnt_EN&&cntReg===0.S) {
      io.dOut(0) := out1(0)
      io.dOut(1) := out1(1)
      io.dOut(2) := out1(2)
      io.dOut(3) := out1(3)
    }.elsewhen(cnt_EN&&cntReg===1.S) {
      io.dOut(0) := out2(0)
      io.dOut(1) := out2(1)
      io.dOut(2) := out2(2)
      io.dOut(3) := out2(3)
    }.elsewhen(cnt_EN&&cntReg===2.S){
      io.dOut(0) := out3(0)
      io.dOut(1) := out3(1)
      io.dOut(2) := out3(2)
      io.dOut(3) := out3(3)
    }.elsewhen(cnt_EN&&cntReg===3.S){
      io.dOut(0) := out4(0)
      io.dOut(1) := out4(1)
      io.dOut(2) := out4(2)
      io.dOut(3) := out4(3)
...
class Add_20in_21out_IP extends Bundle with Config{
  val A=Input(SInt(20.W))
  val B=Input(SInt(20.W))
  val S=Output(SInt(21.W))
  val CLK=Input(Clock())
}

class Add_20in_21out extends BlackBox{
  val io=IO(new Add_20in_21out_IP)
}

object Add_20in_21out{
  def apply(op1: SInt, op2: SInt):SInt = {
    val inst = Module(new Add_20in_21out)
    inst.io.A := op1
    inst.io.B := op2
    inst.io.CLK:=clock
    inst.io.S
...
class AT_IO extends Bundle with Config{
  ...
  val  dIn = Input(Vec(MAC_WIDTH,SInt((2*MUL_WIDTH).W)))
  val  dOut = Output(Vec(MAC_WIDTH/2,SInt((2*MUL_WIDTH+2).W)))
...
class AT extends Module with Config{
  ...
  io.dOut(0):=Add_20in_21out(Add_20in_21out(io.dIn(1),io.dIn(2)),ShiftRegister(io.dIn(3),1))
  io.dOut(1):=Add_20in_21out(Add_20in_21out(io.dIn(2),-io.dIn(1)),-ShiftRegister(io.dIn(0),1))
}

class AT_module_IO extends Bundle with Config{
  val mac_in_valid=Input(Bool())
  val  dIn = Input(Vec(MAC_WIDTH,SInt((2*MUL_WIDTH).W)))
  val  dOut = Output(Vec(MAC_WIDTH,SInt((2*MUL_WIDTH+2).W)))
  val  out_valid=Output(Bool())
}
